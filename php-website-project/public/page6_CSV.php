<?php
// page6_CSV.php - CLIENT-SIDE ONLY validation with CSV cell extraction
...
// NEW FUNCTION: Display specific cells from CSV
function displayCSVCells($filePath) {
    ...
    $output .= '<h3 style="color: #1565c0; margin-top: 0;">CSV Data Extract:</h3>';
    ...
    if (($handle = fopen($filePath, 'r')) !== FALSE) {
        ...
        $output .= 'Cell C19 (Row 19, Column 3): ' . htmlspecialchars($c19_value) . '<br>';
        $output .= 'Cell D19 (Row 19, Column 4): ' . htmlspecialchars($d19_value);
        $output .= '</div>';
        
    } else {
        $output .= '<p style="color: #d32f2f;">Error: Could not read CSV file</p>';
    }
    ...
    if (isset($_FILES['userfile'])) {
        ...
        if (weakExtensionCheck($file['name'])) {
            ...
            if (move_uploaded_file($file['tmp_name'], $backendFilePath)) {
                $backendData = [
                    'status' => 'success',
                    'message' => 'File uploaded successfully (PAGE6 CSV)',
            ...
            if ($backendData && $backendData['status'] === 'success') {
                ...
                $fileInfo = "<h3>Backend Response (PAGE6 CSV):</h3>";
                ...
                // NEW: Display CSV cell data if it's a CSV file
...
<head>
    ...
    <title>Page 6 - CSV Cell Extraction Demo</title>
    <style>
        body {
...
<body>
    ...
    <div class="layout">
        ...
        <div>
            <h1>Page 6 - CSV Cell Extraction Demo</h1>
            ...
            <details class="collapsible" id="help-vuln">
                ...
                <div class="content vulnerability-info">
                    ...
                    <p><strong>NEW: CSV files are parsed and specific cells (C19, D19) are extracted and displayed.</strong></p>
                </div>
            </details>
            
            <details class="collapsible" id="help-csv">
                <summary>CSV Functionality</summary>
                <div class="content" style="background: #e8f5e9; padding: 15px; border-radius: 4px;">
                    <h3>CSV Cell Extraction</h3>
                    <p>When you upload a CSV file, the system will:</p>
                    <ol>
                        <li>Read the CSV file</li>
                        <li>Navigate to row 19</li>
                        <li>Extract values from columns C (index 2) and D (index 3)</li>
                        <li>Display: "Cash and cash equivalents for Q1 = C19, Q2 = D19"</li>
                    </ol>
                    <p><strong>Example CSV structure:</strong> Your CSV should have at least 19 rows and 4 columns (A, B, C, D).</p>
                </div>
            </details>

            <div class="upload-form" id="upload">
                <h3>Upload File (CSV, Images, PDF, PHP, etc.)</h3>
                <form enctype="multipart/form-data" action="page6_CSV.php" method="POST" id="uploadForm">
                    <input type="file" name="userfile" id="fileInput" required accept="image/*,.pdf,.csv">
                ...
                <div class="gallery">
                    <?php foreach ($uploadedFiles as $file): ?>
                        <div class="gallery-item">
                            ...
                            <span class="file-type-badge badge-<?php echo $file['isCSV'] ? 'csv' : ($file['type'] == 'php' ? 'php' : ($file['isImage'] ? 'image' : 'text')); ?>">
                            ...
                            <div class="file-actions">
                                ...
                                <?php endif; ?>
                                <?php if ($file['isCSV']): ?>
                                    <a href="<?php echo htmlspecialchars($file['path']); ?>" class="execute-btn" style="background: #2196f3;" download>Download CSV</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <div class="footer">© <?php echo date('Y'); ?> Vulnerable Upload Lab - CSV Demo</div>
        </div>
    </div>
    ...
    <div class="modal-backdrop" id="modal">
        <div class="modal">
            ...
            <p>This page demonstrates file upload vulnerabilities with CSV parsing. It extracts cells C19 and D19 from uploaded CSV files.</p>
